<%@ page contentType="text/html; charset=UTF-8" pageEncoding="UTF-8"%>
<%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core"%>
<%@ page import="com.chat.model.*"%>

<%
	// 從 request 取得 Controller 傳過來的 chatMessageVO
	// (可能是 getOne_For_Update 傳來的，也可能是 update 失敗後傳回來的)
	ChatMessageVO chatMessageVO = (ChatMessageVO) request.getAttribute("chatMessageVO"); 
%>

<html>
<head>
<meta http-equiv="X-UA-Compatible" content="IE=edge,chrome=1"/>
<title>對話訊息修改 - update_chat_input.jsp</title>

<style>
  table#table-1 {
	background-color: #CCCCFF;
    border: 2px solid black;
    text-align: center;
  }
  table#table-1 h4 {
    color: red;
    display: block;
    margin-bottom: 1px;
  }
  h4 {
    color: blue;
    display: inline;
  }
</style>

<style>
  table {
	width: 600px;
	background-color: white;
	margin-top: 1px;
	margin-bottom: 1px;
  }
  table, th, td {
    border: 0px solid #CCCCFF;
  }
  th, td {
    padding: 1px;
  }
</style>

</head>
<body bgcolor='white'>

<table id="table-1">
	<tr><td>
		 <h3>對話訊息修改 - update_chat_input.jsp</h3>
		 <h4><a href="<%=request.getContextPath()%>/back_end/chat/select_page.jsp"><img src="<%=request.getContextPath()%>/resources/images/back1.gif" width="100" height="32" border="0">回首頁</a></h4>
	</td></tr>
</table>

<h3>資料修改:</h3>

<%-- 錯誤表列 --%>
<c:if test="${not empty errorMsgs}">
	<font style="color:red">請修正以下錯誤:</font>
	<ul>
		<c:forEach var="message" items="${errorMsgs}">
			<li style="color:red">${message}</li>
		</c:forEach>
	</ul>
</c:if>

<FORM METHOD="post" ACTION="<%=request.getContextPath()%>/chat/chat.do" name="form1">
<table>
	<%-- 訊息ID - 不可修改 --%>
	<tr>
		<td>訊息編號:<font color=red><b></b></font></td>
		<td>${chatMessageVO.messageId}</td>
	</tr>
	
	<%-- 聊天室ID - 不可修改 --%>
	<tr>
		<td>聊天室編號:</td>
		<td>${chatMessageVO.chatroomId}</td>
	</tr>

	<%-- 會員 ID - 不可修改 --%>
	<tr>
		<td>會員編號:</td>
		<td>${chatMessageVO.memberId}</td>
	</tr>
	
	<%-- 訊息內容 - 可修改 --%>
	<tr>
		<td>訊息內容:</td>
		<td>
			<input type="TEXT" name="message" value="${chatMessageVO.message}" size="50"/>
		</td>
	</tr>
	
	<%-- 聊天時間 - 顯示用 --%>
	<tr>
		<td>發送時間:</td>
		<td>${chatMessageVO.chatTime}</td>
	</tr>

</table>
<br>
<%-- 隱藏欄位：告訴 Servlet 這是 update 動作，以及要修改哪一筆 ID --%>
<input type="hidden" name="action" value="update">
<input type="hidden" name="messageId" value="${chatMessageVO.messageId}">

<input type="submit" value="送出修改"></FORM>
</body>
</html>